...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::fprintf(stderr, "Usage: %s <host> <port> [connections=%u] [threads=auto] [duration=%us]\n",
    ...
    char tmp[1024];
    int  tlen = std::snprintf(tmp, sizeof(tmp), HTTP_FMT, cfg.host.c_str());
    std::string payload(tmp, tlen > 0 ? tlen : 0);
    size_t      plen = payload.size();

    std::string pipe_buf;
    pipe_buf.reserve(plen * PIPELINE);
    for (int i = 0; i < PIPELINE; ++i) pipe_buf += payload;

    std::vector<iovec> iov(PIPELINE);
    for (int i = 0; i < PIPELINE; ++i) {
        iov[i].iov_base = const_cast<char*>(pipe_buf.data() + i * plen);
        iov[i].iov_len  = plen;
    }
